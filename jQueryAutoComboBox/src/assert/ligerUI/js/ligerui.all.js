...
(function ($)
{
    ...
    $.ligerDefaults.Grid.sorters['float'] = function (val1, val2)
    {
        if(isNaN(parseFloat(val1)) && !isNaN(parseFloat(val2)))
		 return -1;
		else if(!isNaN(parseFloat(val1)) && isNaN(parseFloat(val2)))
		 return 1;
		else if(isNaN(parseFloat(val1)) && isNaN(parseFloat(val2)))
		 return 0;
		else
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        //改变排序,加flag,true的时候每次赋值新值12.09修改zt
         changeSort: function (columnName, sortOrder,flag)
        {
			var g = this, p = this.options;
        ...
        _onClick: function (e)
        {
		  var obj = (e.target || e.srcElement);
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
				...
				if(p.enabledEdit && !src.checkbox && !selectRowButtonOnly)
				{
				   g.unselect(src.data);
				   g.select(src.data);
				}//ztend
                else if(src.checkbox || !selectRowButtonOnly)
                {
					var row = $(src.row);
...
(function ($)
{
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        removeTabItem: function (tabid)
        {
		  ...
		  //qj
		  $("a[tabid='"+tabid+"'] span").removeClass("l-link-open");
...
(function ($)
{
    ...
    $.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item)
        {
			...
			//qjst
			//item.display&&ditem.addClass("l-toolbar-item-display-"+item.display);					  			
			//qjend	
            item.text && $("span:first", ditem).html(item.text);
